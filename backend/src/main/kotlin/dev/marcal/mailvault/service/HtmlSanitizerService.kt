...
class HtmlSanitizerService {
    private val policy: PolicyFactory =
        HtmlPolicyBuilder()
            .allowElements(
                ...
                "blockquote",
                "hr",
            ).allowAttributes("data-safe-href")
            .matching(SAFE_LINK_PATTERN)
            .onElements("a")
            .allowAttributes("title")
            .onElements("a")
            .allowAttributes("data-safe-src")
            .matching(SAFE_IMAGE_PATTERN)
            .onElements("img")
            .allowAttributes("data-original-src")
            .matching(SAFE_REMOTE_PATTERN)
            .onElements("img")
    ...
    private companion object {
        ...
        val SAFE_IMAGE_PATTERN: Pattern =
            Pattern.compile(
                "(?i)^(/api/messages/.+/cid/.+|/assets/.+/.+|/static/remote-image-blocked\\.svg)$",
            )
